...
public class ProductMarkView extends EntryView {
	...
	public ProductMarkController getController() {

		return (ProductMarkController) super.getController();

	}


	public ProductMarkView(ProductMarkController controller) {
	...
	public DBIdentifierField getIdentifierField() {
		...
		if (identifierField == null){
			...
			identifierField = new DBIdentifierField(new IdentifierTag(), ProductMarkController.IDENTIFIER_PROPERTY);
	...
	public DBTextField getDenominationField() {
		...
		if (denominationField == null){
			...
			denominationField = new DBTextField(new DenominationTag(), ProductMarkController.DENOMINATION_PROPERTY);
	...
	public DBTextField getShortDenominationField() {
		...
		if (shortDenominationField == null){
			...
			shortDenominationField = new DBTextField(new ShortDenominationTag(), ProductMarkController.SHORT_DENOMINATION_PROPERTY);
	...
	public Search getIdentifierSearch(){
		...
		if (identifierSearch==null){
			...
			identifierSearch = new ProductMarkSearch();

			identifierSearch.addSearchListener(new SearchAdapter(){

				public void onSearchConfirmed(SearchEvent event) throws Exception {

					getController().changeIdentifierProperty(((ProductMark)event.getSelectedRows().get(0)).getIdentifier());
